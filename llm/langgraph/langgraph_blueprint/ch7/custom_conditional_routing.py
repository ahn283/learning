...
# Define a node to handle basic artithmetic calculations
...
def routing_function(state: MessagesState):
    ...
    if 'weather' in last_message:
        ...
        return "calculator_node"    # Route to default node for uncategorized inputs
    
    return "default_node"           # Route to default node for uncategorized inputs
...
builder.add_node("routing_node", routing_function)

# Set up the edges for routing
builder.add_conditional_edges(START, routing_function)      # Routing based on the routing function
builder.add_edge("weather_node", END)       # Route to end after node
...
# Simulate Interaction with the agents
...
# Simulate interacting with the agent
