...
def identify_product(issue: str) -> str:
    """Identifies the product baased on the issue description."""
...
def escalate_to_support(product: str, issue: str) -> str:
    """Escalate the issue to a human support team."""
    if not product or not issue or not isinstance(product, str) or not isinstance(issue, str):
        return "Error: Invalid product or issue provided"
...
SYSTEM_PROMPT = """You are an customer support assistant. Your task is to help customers troubleshooting product issues using the available tools.
...
2. search_manual - Seaches the product manual for troubleshooting steps
...
async def replan_step(state: PlanExecute) -> dict:
    try:
        ...
        replanner_prompt = ChatPromptTemplate.from_template("""
                                                            ...
                                                            2. Provide a final response if the work flow is complete
...
config = {"configurable": {"thread_id":"1"}, "rescursion_limit": 50}
