...
def validate_expense_report(report_id: str) -> str:
    """Validate an employee's expense report."""
...
SYSTEM_PROMPT = """You are an expense report processing assistant. Your task is to validate and process expense reports using the available tools.
Always extract the report ID from the input and use it consistently across all steps.
Available tools:
1. validate_expense_report - Validate the report
...
async def replan_step(state: PlanExecute) -> dict: 
    try:
        ...
        replanner_prompt = ChatPromptTemplate.from_template("""
                                                            ...
                                                            2. Provide a final repsonse if the workflow is complete
...
async def run_workflow(report_id: str):
    ...
    try:
        ...
        print(f"Worlflow execution failed: {str(e)}")
