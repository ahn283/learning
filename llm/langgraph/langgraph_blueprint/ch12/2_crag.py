...
vectorstore = Chroma.from_documents(
    doc_splits, collection_name='crag-chroma',
...
retrieved_prompt = ChatPromptTemplate.from_template(
    """
...
retrieval_grader = retrieved_prompt | ChatOpenAI(model='gpt-4o-mini', temperature=0, api_key=OPENAI_API_KEY).with_structured_output(GradeDocuments)
...
re_write_prompt = ChatPromptTemplate.from_template(
    "Here is the inital question: \n\n {question} \n Formualte an improved question."
)

query_rewriter = re_write_prompt | ChatOpenAI(model='gpt-4o-mini', temperature=0, api_key=OPENAI_API_KEY).with_structured_output(ImproveQuestion)
...
rag_chain = prompt | ChatOpenAI(model='gpt-4o-mini', temperature=0, api_key=OPENAI_API_KEY) | StrOutputParser()
...
def web_search(state):
    ...
    # Process results to create Document object only with page_content
    web_document = [
...
# Example input
inputs = {"question": "Explain why the different types of agent memory work?"}
