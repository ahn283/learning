...
newsletter_generator = ChatAnthropic(model_name='claude-3-5-sonnet-latest')
...
def generate_newsletter_theme(state: State) -> State:
    ...
    In addition, generates 5 sub-themes that are highly specific, reseachable news items or insights under the main theme.
    ...
    The sub-themse should create a clear direction for the newsletter, avoiding broad, generic topics.
    ...
    # chain togehter the system prompt and the structured output model
...
async def write_newsletter_section_async(state: State, sub_theme: str) -> Dict:
    ...
    Use the following articles as reference and include points from both their titles, images, and content:
    <article>
    {article_references}
    <article/>
...
async def edit_newsletter(state: State) -> State:
    theme = state['newsletter_theme'].theme
    combined_newsltter = state['messages'][-1].content
    ...
    1. Consistent tone and style throughout the newletter
