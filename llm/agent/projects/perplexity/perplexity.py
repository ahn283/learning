...
WOLFRAM_ALPHA_APPID = keyring.get_password('wolfram', 'key_for_windows')
...
# build a graph
graphh_builder = StateGraph(State)
graphh_builder.add_node("chatbot", chatbot)

for focus, tool_node in tool_nodes.items():
    graphh_builder.add_node(f"{focus}_tools", tool_node)
...
graphh_builder.add_conditional_edges(
    "chatbot",
    tools_condition,
    {
        "web_tools": "web_tools",
...
for focus in tools:
    graphh_builder.add_edge(f"{focus}_tools", "chatbot")
    
graphh_builder.set_entry_point("chatbot")
graph = graphh_builder.compile()
...
selected_focus = []
for area, key in focus_area.items():
...
if prompt := st.chat_input("Type you message here"):
    if not selected_focus:
        ...
        with st.status("Processing you request...", expanded=True) as status:
            responses = []
