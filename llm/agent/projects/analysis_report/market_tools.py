...
def get_latest_filing_content(ticker: str) -> dict: 
    """collect recetn 10-K, 10-Q, 8-k filing and extract contents from them of a given ticker"""
    headers = {"User-Agent": "ahn283@gmail.com"}
    
    # get company tickers and ciks
    ...
    company_data['cik_str'] = company_data['cik_str'].astype(str).str.zfill(10)     # 10-digits
    ticker = ticker.upper()
    ...
    # summaries for filings and collections
...
def collect_competitor_news(ticker, news_count, interval=10):
    """Collect related companies' news about a given ticker company
    ...
    print("completed getting competitors")
    time.sleep(10)
    ...
    for c in competitors:
        try:
            api_url = f"https://api.polygon.io/v2/reference/news?ticker={c}&order=desc&limit={news_count}&sort=published_utc&apiKey={POLYGON_API_KEY}"
            result = requests.get(api_url).json()
            competitors_news[c] = [i['description'] for i in result['results']]
            time.sleep(10)
            print(f"Completed news downlod competitor: {c}")
        except Exception as e:
            print(f"Failed downloading news competitor {c}\n Error: {e}")
