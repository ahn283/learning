...
#3. set analyst agents and prompts
financial_prompt = f"""
오늘은 {today}입니다. 
...
#4. create ReAct agents
financial_agent = create_react_agent(
    llm,
    [get_financial_data, analyze_data, chart_generator],
    state_modifier=financial_prompt
)
stock_agent = create_react_agent(
    llm, 
    [fetch_stock_data, analyze_data, chart_generator],
    state_modifier=stock_prompt
)
market_agent = create_react_agent(
    llm,
    [get_latest_filing_content, collect_company_news, collect_competitor_news,
     collect_market_news, scrape_webpages],
    state_modifier=market_prompt
)

#5. Map function: assign tasks to each analyst
...
#6. Analyst node function
def analyst_node(state: dict, agent, task_type: str):
    """execute each analyst and structure the output"""
    company = state['company']
    result = agent.invoke(
...
#7. Reduce function: integration of analyses
...
# add a conditional edge from START to map_to_analyst
...
# connect analyses results into combine
