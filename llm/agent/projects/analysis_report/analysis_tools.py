...
BASE_DIR = Path(os.getcwd())    # current directory
DATA_DIR = BASE_DIR / 'data'    # a directory for data
CHART_DIR = BASE_DIR / 'chart'  # a ditectory for charts

@tool
def fetch_stock_data(ticker: str, start_date: str, end_date: str):
    """Fetch stock data and compare with NASDQA and competitors."""
    
    POLYGON_API_KEY = keyring.get_password('polygon', 'key_for_windows')
    ...
    try:
        ...
        if stock.empty:
            raise ValueError(f"No data fetched for {ticker}. Check the ticker or date range.")
...
@tool
def get_financial_data(ticker: str, num_years: int, interval = 10):
    """ 
    Get financial data, change into DataFrame, and return the file path
    ...
    # collect data of recent n years financial data
    financials = []
    ...
    return f"Fincancial data saved to {csv_path}" 
...
def chart_generator(command: str):
    """this tool creates charts and saves into /charts folder using create_pandas_dataframe_agent"""
    ...
    stock_path is './data//stock_data.csv'.
    finance_data is './data/finance_data.csv'
    """
